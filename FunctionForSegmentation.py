...
def Segmentation(WorkingDirectory, ListTrainingDataFile, ListImageName, modelname, noiseReduction, numberOfClasses, classesNamesList, ROI, ListAreaNames, fusionClassesY_N, maskY_N, imageY_N, InfoY_N, NFMaskY_N, chosenArea, ReferencePicture):
    ...
    Args:
        ...
        modelname (string): 
            ...
            ('Support Vector Machine (Sklearn)','Random Forest Classifier (Sklearn)','Classification and Regression Tree (Sklearn)', 'Support Vector Machine (OpenCV)' or 'K-Nearest Neighbors (OpenCV)')
        ...
        NFMaskY_N (string): 
            Two possible values 'Y' or 'N'. wo possible values 'Y' or 'N'. If maskY_N='Y', the user have chosen to save the mask before any noise reduction and morphological filtering.  
...
def ApplyModelAndSaveOutput(model, modelname, imageArray, ImageName,NameArea, noiseReduction, numberOfClasses, classesNamesList, fusionClassesY_N, maskY_N, InfoY_N, imageY_N, NFMaskY_N, chosenArea, OutputMaskName, OutputimageName, OutputNFMaskName, ListAirs, ListTestDataTimes,ListApplyModelTimes,ListSaveOutputTimes):
    """ Main function for the segmentation.
    ...
    if numberOfClasses>2 and fusionClassesY_N=='N' :
        ...
        if contours!=[]:
            ...
            if imageY_N=='Y':
                ...
                FilteredMask3[FilteredMask2==0]=0.3            
...
def TrainModel(trainData, modelname,classesNamesList):
    ...
    if modelname=='Support Vector Machine (OpenCV)':
        model = cv2.ml.SVM_create()
        model.train(trainData, cv2.ml.ROW_SAMPLE , ResponseNumber)
    
    if modelname=='K-Nearest Neighbors (OpenCV)':
...
def ApplyModel(testData, modelname, model):
    ...
    if modelname=='K-Nearest Neighbors (OpenCV)':
        ret, result, neighbours, dist = model.findNearest(testData, 10)
